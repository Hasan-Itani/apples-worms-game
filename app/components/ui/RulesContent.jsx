...
const H2 = memo(function H2({ id, children }) {
  return (
    <h2
      id={id}
      className="mt-3 mb-1 font-extrabold text-sky-300 text-center text-lg"
    >
      {children}
    </h2>
  );
});
const P = memo(function P({ children }) {
  return <p className="text-sm leading-relaxed text-center">{children}</p>;
});
const L = memo(function L({ children }) {
  return <li className="text-sm leading-relaxed">{children}</li>;
});
const M = memo(function M({ children }) {
  return <div className="text-xs opacity-80 text-center">{children}</div>;
});

/**
 * RulesContent — static rules/help for Apples & Worms.
 *
 * Accessibility & semantics:
 * - Real headings (<h2>) with ids for anchor linking / screen readers
 * - Top-level sections wrapped in <section> with aria-labelledby
 * - Standard <ul><li> markup retained; helper components are memoized
 */
export default function RulesContent() {
  return (
    <div className="w-full max-w-[720px] mx-auto text-white">
      <section aria-labelledby="rules-title">
        <H2 id="rules-title">Apples & Worms™</H2>
        <P>
      ...
      <section aria-labelledby="play-title">
        <H2 id="play-title">To play the game:</H2>
      ...
      <section aria-labelledby="manual-title">
        <H2 id="manual-title">Manual Game Flow:</H2>
        <ul className="list-disc list-inside space-y-1 text-left">
      ...
      <section aria-labelledby="bankit-title">
        <H2 id="bankit-title">Bank It Option:</H2>
        <ul className="list-disc list-inside space-y-1 text-left">
          ...
          <L>
            Please note: When using the Bank It option, the current winning
            value in the blue cell can't be lower than a minimal bet.
          </L>
          <L>
      ...
      <section aria-labelledby="autoplay-title">
        <H2 id="autoplay-title">Autoplay:</H2>
        <ul className="list-disc list-inside space-y-1 text-left">
          <L>Press the Auto button to switch to the Autoplay menu.</L>
          <L>
            You can set the Bet Amount, number of Worms, and the size of the
            field as in the regular gameplay.
          </L>
          <L>
          ...
          <L>
            Click the tiles on the game field one by one to set the order in
            which tiles will reveal their symbols during the Autoplay session.
          </L>
          <L>
      ...
      <section aria-labelledby="details-title">
        <H2 id="details-title">Game Details:</H2>
        <ul className="list-disc list-inside space-y-1 text-left">
          <L>Game Details section shows the current game stats.</L>
          <L>
            Maximum Win field shows the maximum possible win for the game with
      ...
      <section aria-labelledby="maxwin-title">
        <H2 id="maxwin-title">Maximum win limit:</H2>
        <P>
      ...
      <section aria-labelledby="rtp-title">
        <H2 id="rtp-title">Return to Player:</H2>
        <P>
          The minimum theoretical percentage return to player (RTP) is 87.50%
          and the maximum theoretical percentage return to player (RTP) is
          97.20%. The RTP value is the theoretical return to player, calculated
          by dividing the total winnings by total amount bet from 1000000000
          simulated game rounds.
        </P>
      </section>
      <BlueDivider />

      <section aria-labelledby="disconnect-title">
        <H2 id="disconnect-title">Note on disconnections:</H2>
        <P>
          If you are disconnected from the Internet in the middle of the game,
          log in again into the casino. You will be automatically directed back
          to the game, and you can continue the game from the point where it was
          broken. If you re-open the game without re-logging into the casino,
          the game will start from the very beginning. In both cases, your
          previous wins will be paid.
        </P>
