 ...
 *
 * Props:
 * - onStart: function called when the user clicks the start button
 *
 * Features:
 * - Background image + centered logo
 * - Start button with hover/pressed background swap
...
export default function Loading({ onStart }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("loading_dontShow");
    if (saved === "true") {
      setDontShow(true);
    }
  }, []);
  ...
  const handleToggleDontShow = () => {
    const next = !dontShow;
    setDontShow(next);
    localStorage.setItem("loading_dontShow", next);
  };
